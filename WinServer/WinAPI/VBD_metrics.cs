namespace WinAPI
{
    ...
    public class VBD_metrics : XenObject<VBD_metrics>
    {
        ...
        {
            session.proxy.vbd_metrics_set_other_config(session.uuid, (_vbd_metrics != null) ? _vbd_metrics : "", Maps.convert_to_proxy_string_string(_other_config)).parse();
        }

        public Proxy_VBD_metrics ToProxy()
        {
            return new Proxy_VBD_metrics { uuid = (this.uuid != null) ? this.uuid : "", io_read_kbs = this.io_read_kbs, io_write_kbs = this.io_write_kbs, last_updated = this.last_updated, other_config = Maps.convert_to_proxy_string_string(this.other_config) };
        }
